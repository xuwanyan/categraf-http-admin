...
add_one '{"url":"http://192.0.2.20:51233/third/info","job":"东电接口-api接口"}' "东电接口-api接口"
...
add_one '{"url":"http://jsonws.example.com:8880/JSONWS/","job":"HMGWS-246主机"}' "HMGWS-246主机"
...
echo ""
echo "=== 第2组：Carrier POST 目标（自定义配置） ==="

add_one '{"url":"https://gateway.example.com/api/example/external/direct/EXAMPLE","job":"carrier-国外接口","method":"POST","expected_status_codes":"400","response_timeout":"15s","body":"{}","headers":["Content-Type","application/json"],"use_tls":true,"tls_ca":"/etc/categraf/ssl/example_ca.pem"}' "carrier-国外接口(POST)"

echo ""
echo "=== 全部导入完成 ==="
echo ""
echo "验证：curl -s http://127.0.0.1:5000/api/targets | python3 -m json.tool | grep -c '\"url\"'"
echo "确认目标数后，停掉 categraf 备份本地配置："
